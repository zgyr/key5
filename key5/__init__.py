...
# Use left and right arrow keys for navigation, down arrow for delete previous character, up arrow to enter character input mode.

or

key5.roll('Hello, world!')

# And just select a character from the roll, confirm the input with the enter key.
'''
import keyboard
import importlib
...
def roll(string = '', cfg = 'key5.layout'):
    ...
    while True:
        ...
        if layout.pos_y == layout.bias_y:
            layout.display = repr(string[:layout.cur] + '\u2588' + string[layout.cur:])
        ...
        if key == 'down':
            ...
            layout.pos_x = max(min(layout.pos_x, len(layout.char_map[layout.pos_y]) - layout.bias_x[layout.pos_y] - 1), -layout.bias_x[layout.pos_y])
        elif key == 'up':
            ...
            if layout.pos_y == layout.bias_y:
                ...
                layout.pos_x = min(layout.pos_x + 1, len(layout.char_map[layout.pos_y]) - c_bias - 1)
        elif key == 'enter':
            if layout.pos_y == layout.bias_y:
                return string
